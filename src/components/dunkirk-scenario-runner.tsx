...
export function DunkirkScenarioRunner() {
  ...
  async function start() {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body?.error ?? "Unable to save this run.");
      }
      ...
      setSaveError(caught instanceof Error ? caught.message : "The run is playable locally but could not be saved for sharing.");
    }
  }
  ...
  async function resolve(choice: string) {
    ...
    try {
      ...
      if (caught instanceof Error && state) {
  ...
  return <section className="runner" id="dunkirk-runner">
    ...
    <div className="runner-grid"><div className="runner-controls">
      <h3>Choose a divergence</h3>{dunkirk1940.basicChoices.map((item) => <button className={choiceId === item.id ? "choice active" : "choice"} key={item.id} onClick={() => { setChoiceId(item.id); resetRun(); }} type="button">{item.label}</button>)}
      <h3>Tune variables</h3>{dunkirk1940.advancedVariables.map((item) => <label className="variable-control" key={item.id}>{item.label}: {values[item.id]}<input max={item.maximum} min={item.minimum} onChange={(event) => { setValues((current) => ({ ...current, [item.id]: Number(event.target.value) })); resetRun(); }} step={item.step} type="range" value={values[item.id]} /></label>)}
